...
export default function ReferencesClient({
  ...
  const totalFilteredRefs = filtered.reduce(
    (acc: number, r: any) => acc + r.references.length,
    0,
  );
    ...
    <div className="w-[95%] lg:w-[90%] max-w-7xl mx-auto pt-4 pb-24 space-y-6">
      ...
      >
        ...
        <div className="relative z-10 px-8 py-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <p className="text-navy-300 text-xs font-semibold uppercase tracking-widest mb-2">
              Hệ thống quản lý
            </p>
            ...
            <p className="text-navy-200/80 text-sm max-w-lg">
              Tổng hợp toàn bộ nguồn tài liệu khoa học được đính kèm trong các
              báo cáo nghiên cứu.
            </p>
          </div>
          <div className="flex gap-3 shrink-0 flex-wrap">
            {[
              ...
              <div
                key={s.label}
                className="bg-white/10 border border-white/10 rounded-2xl px-5 py-3 text-center min-w-[100px]"
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap gap-1.5 mb-1">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-md border ${FORMAT_COLORS[fmtKey] || "bg-slate-100 text-slate-600 border-slate-200"}`}
                      >
                        {report.format?.toUpperCase()}
                      </span>
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${CATEGORY_COLORS[catKey] || "bg-slate-100 text-slate-600"}`}
                      >
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <button
                      ...
                      className="p-2 text-navy-600 hover:bg-navy-100 rounded-lg transition-colors flex items-center gap-1.5 text-xs font-bold mr-2"
                    >
