...
import { FileText, UploadCloud, Trash2, X, Search, FileUp, DownloadCloud, Eye } from 'lucide-react'
...
export default function DocumentsClient({ initialDocuments, session }: { initialDocuments: any[], session: any }) {
  ...
  const uniqueUsernames = Array.from(new Set(documents.map(d => d.uploader?.username).filter(Boolean)))

  const filteredDocs = documents
    .filter(doc => {
      const matchSearch = doc.title.toLowerCase().includes(search.toLowerCase()) || 
                          (doc.description && doc.description.toLowerCase().includes(search.toLowerCase()))
      const matchUser = usernameFilter === 'ALL' || doc.uploader?.username === usernameFilter
    ...
    <div className="w-full max-w-6xl mx-auto pt-4 pb-24 space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden rounded-3xl">
        ...
        <div className="relative z-10 px-6 md:px-8 py-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <p className="text-navy-300 text-xs font-semibold uppercase tracking-widest mb-1">Hệ thống bảo mật</p>
            <h1 className="text-2xl md:text-3xl font-extrabold text-white mb-2">Tài liệu Nội bộ</h1>
            <p className="text-navy-200/80 text-sm max-w-lg">Quản lý các tài liệu kỹ thuật, hướng dẫn và quy trình nội bộ an toàn.</p>
          </div>
          <div className="shrink-0">
            <button onClick={() => setIsUploadOpen(true)} className="flex items-center gap-2 bg-white text-navy-800 px-5 py-3 rounded-xl font-bold text-sm hover:bg-slate-50 transition-colors shadow-sm">
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row gap-3 items-center">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          >
            ...
            {uniqueUsernames.map(uname => (
              <option key={uname as string} value={uname as string}>{uname as string}</option>
            ))}
          </select>
        ...
        <p className="text-sm text-slate-500 whitespace-nowrap hidden lg:block">{filteredDocs.length} tài liệu</p>
      </motion.div>
      ...
      {filteredDocs.length === 0 ? (
        <div className="bg-white rounded-3xl p-16 text-center border border-slate-100 shadow-sm flex flex-col items-center">
          ...
          <p className="text-slate-400 font-medium">Chưa có tài liệu nào được tải lên</p>
        </div>
      ) : (
              ...
              >
                <div className="p-5 flex-1">
                  <div className="flex justify-between items-start mb-3">
                    <div className={clsx("w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border", FORMAT_COLORS[fmtKey] || 'bg-slate-50 text-slate-600 border-slate-200')}>
                      <FileText className="w-5 h-5" />
                    </div>
                    <span className={clsx("text-[10px] font-bold px-2.5 py-1 rounded-md border", FORMAT_COLORS[fmtKey] || 'bg-slate-100 text-slate-600 border-slate-200')}>
                      {doc.format?.toUpperCase().replace('.', '') || 'FILE'}
                    </span>
                  </div>
                  
                  <h3 className="font-bold text-slate-800 text-base line-clamp-1 mb-1">{doc.title}</h3>
                  {doc.description && <p className="text-sm text-slate-500 line-clamp-2 mb-3 h-10">{doc.description}</p>}
                  
                  <div className="flex items-center gap-2 mt-auto pt-2 text-xs text-slate-400">
                    <span className="font-medium bg-slate-50 px-2 py-0.5 rounded-md border border-slate-100">{doc.uploader?.username}</span>
                    <span>•</span>
                ...
                <div className="bg-slate-50 border-t border-slate-100 p-2 flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  {canDelete && (
                    <button 
                      ...
                      title="Xóa tài liệu"
                    >
                      {deletingId === doc.id ? <div className="w-4 h-4 border-2 border-red-200 border-t-red-500 rounded-full animate-spin" /> : <Trash2 className="w-4 h-4" />}
                    </button>
                  )}
                </div>
              </motion.div>
            ...
            >
              <div className="flex justify-between items-center p-4 md:px-6 border-b border-slate-100 bg-slate-50/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-bold text-slate-800 line-clamp-1">{previewDoc.title}</h2>
                    <p className="text-xs text-slate-500">{previewDoc.format?.toUpperCase().replace('.', '') || 'FILE'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <a href={previewDoc.fileUrl} target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-100 hover:bg-slate-200 rounded-xl text-slate-600 transition-colors flex items-center gap-2 text-sm font-semibold px-4 hidden sm:flex">
                    <DownloadCloud className="w-4 h-4" /> Tải xuống
                  </a>
                  <button onClick={() => setPreviewDoc(null)} className="p-2.5 bg-slate-100 hover:bg-red-50 hover:text-red-600 rounded-xl text-slate-500 transition-colors">
                    <X className="w-5 h-5" />
                  </button>
                </div>
              </div>
              ...
              <div className="flex-1 bg-slate-100/50 p-4 md:p-6 overflow-hidden">
                {['.jpg', '.jpeg', '.png', '.gif', '.svg'].includes(previewDoc.format?.toLowerCase()) ? (
                  ...
                  <iframe src={previewDoc.fileUrl} className="w-full h-full bg-white rounded-2xl border border-slate-200 shadow-sm" />
                ) : (
                  <iframe src={`https://docs.google.com/gview?url=${encodeURIComponent(previewDoc.fileUrl)}&embedded=true`} className="w-full h-full bg-white rounded-2xl border border-slate-200 shadow-sm" />
                )}
              </div>
            </motion.div>
          </div>
            ...
            >
              <div className="p-6 md:p-8">
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-xl font-extrabold text-slate-800">Tải lên Tài liệu</h2>
                  <button onClick={() => setIsUploadOpen(false)} className="p-2 bg-slate-50 hover:bg-slate-100 rounded-full text-slate-400 transition-colors">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <form onSubmit={handleUpload} className="space-y-5">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Tiêu đề tài liệu</label>
                    <input 
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Mô tả ngắn gọn (Tùy chọn)</label>
                    <textarea 
                  ...
                  <div className="pt-4 flex gap-3">
                    <button type="button" onClick={() => setIsUploadOpen(false)} className="flex-1 py-3 text-slate-600 bg-slate-100 rounded-xl font-bold hover:bg-slate-200 transition-colors">
                      Hủy bỏ
                    </button>
                    <button disabled={loading} type="submit" className="flex-1 py-3 bg-navy-700 text-white rounded-xl font-bold hover:bg-navy-800 transition-colors disabled:opacity-70 flex justify-center items-center gap-2 shadow-md shadow-navy-900/10">
                      {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'Tải lên'}
                    </button>
                  </div>
                </form>
              </div>
