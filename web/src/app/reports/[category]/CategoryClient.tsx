...
import {
  BookOpen, Clock, FolderOpen, Search, SlidersHorizontal,
  ChevronLeft, ChevronRight, User, FileText, Eye, X, Layers,
} from 'lucide-react'
...
export default function CategoryClient({
  ...
  // Derive unique uploaders for filter
  ...
  const filteredReports = useMemo(() => {
    let result = reports.filter(r => {
      ...
      const matchSearch =
        ...
        r.uploader.username.toLowerCase().includes(q)
  ...
  return (
    <div className="w-[95%] lg:w-[90%] max-w-7xl mx-auto pt-4 pb-24 space-y-6">
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 w-48 h-48 bg-indigo-900/20 rounded-full blur-2xl pointer-events-none" />

        <div className="relative z-10 px-8 py-10 md:py-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            <div className="w-14 h-14 md:w-16 md:h-16 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center ring-1 ring-white/30 shrink-0">
              <FolderOpen className="w-7 h-7 text-white" />
            </div>
            <div>
              <p className="text-blue-200 text-xs font-semibold uppercase tracking-widest mb-1">Danh mục báo cáo</p>
          ...
          <div className="flex gap-3 shrink-0">
            <div className="bg-white/15 backdrop-blur-sm border border-white/20 rounded-2xl px-5 py-3 text-center min-w-[100px]">
              <p className="text-2xl font-bold text-white">{reports.length}</p>
              <p className="text-blue-200 text-xs font-medium mt-0.5">Tổng</p>
            </div>
            <div className="bg-white/15 backdrop-blur-sm border border-white/20 rounded-2xl px-5 py-3 text-center min-w-[100px]">
              <p className="text-2xl font-bold text-white">{uploaders.length}</p>
              <p className="text-blue-200 text-xs font-medium mt-0.5">Người đăng</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="flex gap-3 items-center">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 transition-all shadow-sm placeholder-slate-400"
            />
            {searchQuery && (
              <button
          ...
          {showFilters && (
            ...
            >
              <div className="bg-white border border-slate-200 rounded-2xl p-4 grid grid-cols-1 sm:grid-cols-3 gap-4 shadow-sm">
                {/* Format filter */}
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Định dạng</label>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-200 text-sm text-slate-700 rounded-xl px-3 py-2 outline-none focus:border-violet-400 cursor-pointer"
                  >
                ...
                {/* Sort */}
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Sắp xếp</label>
                  <div className="flex gap-2">
                    {[
                      ...
                      <button
                        key={opt.value}
                        onClick={() => setSortBy(opt.value as 'newest' | 'oldest')}
        ...
        <div className="flex items-center justify-between px-1">
          <p className="text-sm text-slate-500">
            <span className="font-semibold text-slate-700">{filteredReports.length}</span> kết quả
            {filteredReports.length !== reports.length && (
              <span className="text-slate-400"> (trong {reports.length} tổng)</span>
            )}
          </p>
        </div>
      </motion.div>
      ...
      {paginatedReports.length === 0 ? (
        ...
        >
          ...
          <p className="text-slate-400 text-sm mb-6 max-w-xs">Thử thay đổi từ khóa hoặc bộ lọc để xem thêm báo cáo.</p>
          <button
            onClick={handleClearFilters}
          ...
          {paginatedReports.map(report => {
            ...
            return (
              <motion.div
                ...
                className="group bg-white rounded-2xl p-5 md:p-6 border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-violet-900/8 hover:border-violet-100 transition-all cursor-pointer flex flex-col gap-4"
              >
                {/* Card top */}
                <div className="flex items-start justify-between gap-3">
                  <div className="flex flex-wrap gap-2 items-center">
                    <span className={`px-2.5 py-1 rounded-lg text-[11px] font-bold uppercase tracking-wide border ${badge}`}>
                      {report.format}
                    </span>
                    <span className="text-xs text-slate-400 font-medium">
                      {format(new Date(report.createdAt), 'dd MMM yyyy', { locale: vi })}
                    </span>
                  </div>
                  <div className="shrink-0 w-8 h-8 rounded-xl bg-slate-50 border border-slate-100 group-hover:bg-violet-50 group-hover:border-violet-100 flex items-center justify-center transition-all">
                    <Eye className="w-4 h-4 text-slate-300 group-hover:text-violet-500 transition-colors" />
                  </div>
                </div>

                {/* Title */}
                <h3 className="font-bold text-base md:text-lg text-slate-800 group-hover:text-violet-700 transition-colors leading-snug line-clamp-2 flex-1">
                  {report.title}
                </h3>
                ...
                <div className="flex items-center justify-between gap-3 pt-3 border-t border-slate-50">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-full bg-gradient-to-br from-violet-500 to-blue-500 flex items-center justify-center text-white text-xs font-bold uppercase shrink-0">
      ...
      {totalPages > 1 && (
        ...
        >
          <button
            ...
            className="p-2.5 rounded-xl bg-white border border-slate-200 text-slate-500 hover:text-violet-600 hover:border-violet-200 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-sm"
          >
